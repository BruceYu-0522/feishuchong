...
| 加分项 | 多 Agent 并行评审、自动回归重试、实时可观测面板、代码语义索引、Pipeline 模板、Git 交付能力、元素圈选修改 |
...
8. 交付总结 Agent 汇总变更、测试结果、风险与 MR 描述草稿。
...
前端包含元素选择器和悬浮对话能力，后端提供 `element-modify-stream` SSE 接口。用户可以在预览页面圈选元素，用自然语言要求 AI 定位并修改源码。
...
Agent 执行层
  ...
  v
运行工作区
  | target-app 副本 / 生成代码 / 测试 / Git 分支 / 导出包
```
...
| `element-picker.js` | 页面圈选、元素上下文采集与对话式修改 |
...
为了方便评委老师下载后直接使用，评审专用 API 配置已随 GitHub 项目文件夹一并提供；运行 `setup_env.ps1` 即可生成本地 `.env` 并启动项目。
...
> 如果仓库公开展示，不建议长期保留真实 API Key。建议使用评审专用临时 Key，并在比赛结束后及时作废。
...
| `GET` | `/pipelines/{id}/element-modify-stream` | 圈选元素后的流式修改接口 |
