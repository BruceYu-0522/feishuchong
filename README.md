...
当前版本是 **API-first + Skill-driven LLM / Mock 双模式**：后端配置 API Key 后会按阶段调用不同模型；未配置 Key 或模型调用失败时，会自动回退到 Mock Agent，保证 Demo 稳定可跑。前端会优先调用 FastAPI 后端，如果后端未启动，则自动回退到浏览器本地 Mock 模式。
...
- 代码生成阶段会在 `devflow-runs/` 中创建真实项目副本并写入代码改动。
- 测试生成阶段会为真实改动生成可运行测试文件。
- 最终形成交付总结和 MR 描述草稿。
...
| 阶段 | Agent | 推荐模型 | 产物 |
|---|---|---|
| 需求分析 | 需求分析 Agent | `deepseek-v4-flash` | 用户故事、功能范围、验收标准 |
| 方案设计 | 方案设计 Agent | `gpt-5.4` | 技术方案、涉及模块、风险点、可视化方案蓝图 |
| 代码生成 | 代码生成 Agent | 本地代码执行器 / `claude-sonnet-4-5-20250929` | 真实项目副本、修改文件列表、diff 摘要 |
| 测试生成 | 测试生成 Agent | 本地测试生成器 / `deepseek-v4-pro` | 真实测试文件、测试说明 |
| 代码评审 | 代码评审 Agent | `claude-opus-4-5-20251101` | 评审结论、风险列表 |
| 交付总结 | 交付总结 Agent | `deepseek-v4-flash` | 变更摘要、测试摘要、MR 描述草稿 |
...
项目内置了一个示例任务管理应用 `target-app/`。当 Pipeline 运行到代码生成阶段时，后端会复制一份到 `devflow-runs/<pipeline-id>/target-app/`，并真实写入“按优先级筛选任务”的代码改动。测试生成阶段会继续在这个运行副本里创建 `tests/priority-filter.test.js`，可以用 Node.js 直接运行验证。
...
如果要启用真实模型调用，先配置中转站 API：

```bash
set DEVFLOW_LLM_ENABLED=true
set DEVFLOW_LLM_BASE_URL=https://api.lingyaai.cn
set DEVFLOW_LLM_API_KEY=你的中转站 API Key
```
...
$env:DEVFLOW_LLM_API_KEY="你的中转站 API Key"
```

不配置 `DEVFLOW_LLM_API_KEY` 时，后端会自动使用 Mock Agent。
...
9. 打开 `devflow-runs/<pipeline-id>/target-app/index.html` 查看真实改动后的目标项目
10. 在运行副本中执行 `node tests/priority-filter.test.js` 验证生成测试
11. 最终审批通过后，生成交付总结
...
├── target-app/                         # 被 DevFlow 修改的示例任务管理项目
├── requirements.txt                    # 后端依赖
├── backend/
│   ├── main.py                         # FastAPI 入口
│   ├── pipeline_engine.py              # 后端 Pipeline 状态机
│   ├── code_executor.py                # 真实代码改动与测试文件生成
